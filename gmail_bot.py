        ...
        try:
            ...
            if is_promotional_or_spam(msg, body):
                ticket_store.mark_failed(
                    message_id,
                    thread,
                    "filtered: promotional/spam classification",
                    conv_id,
                )
            ...
            if conv_id:
                if not freescout_client:
                    raise RuntimeError("FreeScout client not configured for append")
                freescout_client.add_customer_thread(
                    conversation_id=int(conv_id),
                    text=body or snippet or "(no body)",
                    imported=True,
                )
            ...
            conv_id = _extract_conversation_id(ticket)
            if not conv_id:
                raise RuntimeError("Missing conversation id from FreeScout response")
