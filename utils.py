...
def serialize_custom_fields(field_map: Dict[int, Any]) -> list[dict]:
    """Return FreeScout custom fields as ``[{"id": int, "value": str}, ...]``.

    Keys may arrive as strings from configuration. Entries with empty or ``None``
    values are dropped. The resulting list is safe to pass directly to FreeScout's
    ``customFields`` payload key.
    """

    serialized = []
    for field_id, value in (field_map or {}).items():
        if value in {None, ""}:
            continue
        try:
...
def create_ticket(
    ...
    *,
    ...
    customer_name, customer_email = parseaddr(sender)
    ...
    serialized_fields = serialize_custom_fields(custom_fields)
